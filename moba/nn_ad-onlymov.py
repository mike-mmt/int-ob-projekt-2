...
weights_folder_path = 'weights/attackers-defenders/only-mov-4'
...
reward_function_defender = {
    ...
    "timeSpentAwayBase": 100, # x 150 = -150
    # "damageTaken": 0,
    # "friendlyFire": -2,
    # "healEnemy": -1.5,
    "fallDamageTaken": -200,
...
for e in range(100000):
    ...
    if env.mode == 'train':
        ...
        for i in range(0, env.n_agents-6, 6):
            attacker_reward_n.extend(reward_n[i:i+3])
            attacker_networks.extend(networks[i:i+3])
            ...
            defender_reward_n.extend(reward_n[i+3:i+6])
            defender_networks.extend(networks[i+3:i+6])
        ...
        print('top attacker reward:', reward_n[top_attacker_network_i], 'top defender reward:', reward_n[top_defender_network_i])
